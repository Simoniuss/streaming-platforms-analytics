...
class SubsLinePlot extends React.Component {
    ...
    constructor(props){
        ...
        this.plotSubscriptions = this.plotSubscriptions.bind(this)
    }

    componentDidMount() {
        this.props.subs? this.plotSubscriptions() : console.log('no data')
    }
   ...
   componentDidUpdate() {
    ...
    plotSubscriptions() {
        ...
        const data = this.props.platform? this.props.subs.filter(d => d.platform === this.props.platform)
            : this.props.subs

        const x_values = [...new Set(data.map(d => d.quarter))].sort()

        const y_max = max(data, d => d.subs)/scale
        ...
        const dataReady = platform.map( function(plat) {
            return {
                ...
                values: data.filter(d => d.platform === plat)
                    .map( function(d) {
                        return {quarter: d.quarter, subs: d.subs/scale} })
            }
        })
        ...
        select(node)
            ...
            .selectAll("myPoints")
                ...
                .append("circle")
                    ...
                    .attr("cy", function(d) { return y(d.subs) } )
                    .attr("r", 6)
                    .attr("stroke", "none")
        ...
        select(node)
            ...
            .enter()
                ...
                .append("text")
                    ...
                    .attr("transform", function(d) { return "translate(" + x(d.value.quarter) + "," + y(d.value.subs) + ")"; })
